	...
	return (
		<div className="flex items-center gap-2">
			{socialLinkConfig.map((config) => {
				const href = config.isEmail
					? `mailto:${personalInfo.email}`
					: personalInfo[config.key];
				...
				return (
					<motion.a
						...
						className="flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:text-foreground hover:bg-muted"
