...
export function MotionCarousel({ children, className, showDots = true, showNavigation = true }: MotionCarouselProps) {
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      {/* Navigation buttons */}
      {showNavigation && (
        <div className="pointer-events-none absolute top-1/2 right-0 left-0 flex -translate-y-1/2 justify-between px-2 md:-mx-16">
          <motion.button
            type="button"
            onClick={scrollPrev}
            className="glass pointer-events-auto flex h-12 w-12 items-center justify-center rounded-full text-foreground transition-colors hover:bg-glass-hover"
